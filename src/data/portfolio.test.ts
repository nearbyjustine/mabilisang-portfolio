...
describe("portfolio content", () => {
	it("leads with current leadership and measurable proof", () => {
		...
		expect(proofPoints.map((point) => point.value)).toEqual(["4", "3", "35k+"]);
	});
...
describe("featured projects", () => {
	it("ships exactly the four current case studies", () => {
	...
	it("uses in-page anchors for private work and https for live sites", () => {
		for (const project of projects) {
			expect(project.link).toMatch(/^(#|https:\/\/)/);
		}
		expect(projects.filter((project) => project.link.startsWith("#"))).toHaveLength(3);
		expect(projects[3].link).toBe("https://app.centhris.com/");
	});
});
